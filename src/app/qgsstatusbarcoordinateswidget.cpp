...
QString QgsStatusBarCoordinatesWidget::LatLongTopoSheetConversion(const QgsPointXY& mp)
{
  ...
  // 1. Determine IMW Zone (Longitude)
  int zone_num = std::floor((LONG + 180.0) / 6.0) + 1;
  if (zone_num > 60) zone_num = 60;
  if (zone_num < 1) zone_num = 1;
  
  // 2. Determine IMW Band (Latitude)
  double abs_lat = std::abs(LAT);
  int lat_band = std::floor(abs_lat / 4.0);
  if (lat_band > 45) lat_band = 45;
  QChar char1 = matrix1[lat_band];

  // 3. Determine the 1x1 degree block within the 4x6 degree IMW sheet.
  double LONG_START = (zone_num - 31) * 6.0;
  int col = std::floor(LONG - LONG_START);
  if (col < 0) col = 0;
  if (col > 5) col = 5;

  int row;
  if (LAT >= 0) {
    double LAT_START = lat_band * 4.0;
    row = std::floor(LAT - LAT_START);
  } else {
    double LAT_START = -lat_band * 4.0;
    int row_from_north = std::floor(LAT_START - LAT);
    row = 3 - row_from_north;
  }
  if (row < 0) row = 0;
  if (row > 3) row = 3;

  QChar char2 = matrix2[row][col];

  // 4. Determine the 15'x15' cell within the 1x1 degree block.
  int x, y;
  double cell_LONG = LONG_START + col;
  double dLONG = LONG - cell_LONG;
  x = std::floor(dLONG * 4.0);
  if (x < 0) x = 0;
  if (x > 3) x = 3;

  if (LAT >= 0) {
    double cell_LAT = lat_band * 4.0 + row;
    double dLAT = LAT - cell_LAT;
    y = std::floor(dLAT * 4.0);
  } else {
    double cell_LAT_NORTH = -lat_band * 4.0 - (3 - row);
    double cell_LAT_SOUTH = cell_LAT_NORTH - 1.0;
    double dLAT = LAT - cell_LAT_SOUTH;
    y = std::floor(dLAT * 4.0);
  }
  if (y < 0) y = 0;
  if (y > 3) y = 3;
  ...
  arr_ndsm.resize(6);
  arr_ndsm[0] = char1;
  arr_ndsm[1] = QChar('0' + (zone_num / 10) % 10);
  arr_ndsm[2] = QChar('0' + zone_num % 10);
  arr_ndsm[3] = char2;
  arr_ndsm[4] = QChar('0' + (third / 10) % 10);
  arr_ndsm[5] = QChar('0' + third % 10);
